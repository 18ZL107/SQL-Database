...
<main>
    <div class="title"> Here is your vaccination records: <div>
    <?php
        $OHIP = $_POST['PatientOHIP'];

        $sql = "SELECT Vaccination.VaccinationDate, Vaccination.VaccineID, Vaccination.SiteName, Vaccine.CompanyName FROM Vaccination INNER JOIN Vaccine ON Vaccination.VaccineID=Vaccine.LotID and Vaccination.PatientOHIP=$OHIP";
        $result = $connection->query($sql);

        if ($result->rowCount() == 0){
            echo "<div class='title'>No record yet!</div>";
        } else{
            echo "<table><tr><th>VaccinationDate</th><th>VaccineID</th><th>Type</th><th>SiteName</th></tr>";
            while($row = $result->fetch()){
                echo "<tr><td>".$row["VaccinationDate"]."</td><td>".$row["VaccineID"]."</td><td>".$row["SiteName"]."</td><td>".$row["CompanyName"]."</td></tr>";
            }
            echo "</table>";
        }
    ?>
